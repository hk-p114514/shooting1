		...
		if (counter % 100 === 0 && counter < 2800) {
			for (i = 0; i < enemyMaxCount; i++) {
				if (!enemy[i].alive) {
					...
					if (sel2) {
						if (sel === 0) {
							...
							if (j === 2) {
								...
								p.y = screenCanvas.height - enemySize;
							}
						} else {
						...
						if (j % 2 === 0) {
							j = 6;
							p.x = screenCanvas.width + enemySize;
							p.y = screenCanvas.height / 2 - enemySize;
						} else {
							j = 5;
							p.x = -enemySize;
							p.y = screenCanvas.height / 2 - enemySize;
					...
					// タイプに応じて出現座標を設定
					enemy[i].EnemySet(p, enemySize, j);
					// 一体出現させたのでループを終了
					break;
				}
			}
				...
				for (i = 0; i < chara_shot_max_count; i++) {
					if (charaShot[i].alive) {
						for (j = 0; j < enemyMaxCount; j++) {
							if (enemy[j].alive) {
								// 敵キャラクターと自身との距離を測定
								p = enemy[j].position.distance(
									charaShot[i].position
								);
